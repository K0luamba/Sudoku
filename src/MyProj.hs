...
drawGame GameState {gameTable = tab, markPos = (x, y), currentPos = pos, gameOver = state, haveError = err, errorPos = (xe, ye), needHint = flag} = 
  changeView (pictures ( (showCells tab) ++ grid ++ (mark x y) ++ (errorMark err xe ye) ++ (winLabel state) ++ guide ++ (hint flag tab pos) ))
...
handleEvent (EventKey (Char 'h') Down _ _) game@GameState {needHint = state} = game {needHint = not state} --подсказка
handleEvent (EventKey (SpecialKey KeyRight) Down _ _) game@GameState {markPos = (x, y), currentPos = (q, p), haveError = False} --при ошибке блокируем перемещение
    | x < 8 = game {markPos = (x + 1, y), currentPos = moveR (q, p), needHint = False}
    | otherwise = game {markPos = (0, y), currentPos = moveR (q, p), needHint = False}
handleEvent (EventKey (SpecialKey KeyLeft) Down _ _) game@GameState {markPos = (x, y), currentPos = (q, p), haveError = False}
    | x > 0 = game {markPos = (x - 1, y), currentPos = moveL (q, p), needHint = False}
    | otherwise = game {markPos = (8, y), currentPos = moveL (q, p), needHint = False}
handleEvent (EventKey (SpecialKey KeyUp) Down _ _) game@GameState {markPos = (x, y), currentPos = (q, p), haveError = False}
    | y < 8 = game {markPos = (x, y + 1), currentPos = moveU (q, p), needHint = False}
    | otherwise = game {markPos = (x, 0), currentPos = moveU (q, p), needHint = False}
handleEvent (EventKey (SpecialKey KeyDown) Down _ _) game@GameState {markPos = (x, y), currentPos = (q, p), haveError = False}
    | y > 0 = game {markPos = (x, y - 1), currentPos = moveD (q, p), needHint = False}
    | otherwise = game {markPos = (x, 8), currentPos = moveD (q, p), needHint = False}
...
update _ game@GameState {inGame = True, gameOver = True} = game {inGame = False, needHint = False} --разгадан судоку => переход в меню
    ...
    True -> case (checkSize (parseFile fileContent1)) && (checkSize (parseFile fileContent2)) && (checkSize (parseFile fileContent3)) of
      ...
      True -> let tab1 = parseFile fileContent1
              ...
              in do
                ...
                , gameOver = False, haveError = False, errorPos = (0,0), levels = tab1 : tab2 : tab3 : [], needHint = False}
                play display bgColor stepsPerSecond initState drawGame handleEvent update
                printTable tab1
                print $ possible tab1 (9,7)
